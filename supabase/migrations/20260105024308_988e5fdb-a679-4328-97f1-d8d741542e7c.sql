...
-- Criar função de validação para intolerance_key
...
-- Criar trigger para intolerance_safe_keywords também
DROP TRIGGER IF EXISTS validate_safe_keyword_intolerance_key_trigger ON intolerance_safe_keywords;
CREATE TRIGGER validate_safe_keyword_intolerance_key_trigger
  BEFORE INSERT OR UPDATE ON intolerance_safe_keywords
  FOR EACH ROW
  EXECUTE FUNCTION validate_intolerance_key();
...
Prevents phantom keys from being created by AI expansion or bulk imports.';
