-- MIGRATION: Remove dietary preferences, allergies and sensitivities completely
...
-- 4. Update profiles table - set all dietary_preference to 'omnivore'
UPDATE profiles 
...
SELECT 'Profiles with allergies', COUNT(*)
FROM profiles 
WHERE allergies IS NOT NULL AND allergies != '[]'::jsonb

UNION ALL

SELECT 'Profiles with sensitivities', COUNT(*)
FROM profiles 
WHERE sensitivities IS NOT NULL AND sensitivities != '[]'::jsonb;
