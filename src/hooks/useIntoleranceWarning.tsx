  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setDietaryPreference(profileData.dietary_preference || null);
        ...
        let forbiddenData: ForbiddenIngredientItem[] = [];
        ...
        if (hasDietRestriction && userDiet) {
          conditionalFetches.push(
            (async () => {
              const result = await supabase
                .from('dietary_forbidden_ingredients')
                .select('ingredient, dietary_key')
                .eq('dietary_key', userDiet);
              if (result.data) forbiddenData = result.data;
            })()
          );
        }

        // Buscar tabelas pequenas e condicionais em paralelo
        const [dietaryProfilesResult, onboardingResult] = await Promise.all([
          supabase.from('dietary_profiles').select('key, name').eq('is_active', true),
        ...
        // Processar forbidden ingredients
        if (forbiddenData.length > 0) {
          setForbiddenIngredients(forbiddenData);
        }

        // Processar dietary labels
