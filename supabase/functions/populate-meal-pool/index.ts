...
serve(async (req) => {
  ...
  try {
    ...
    // Filter out invalid meals (missing or invalid components)
    const validMeals = generatedMeals.filter((meal) => {
      if (!meal || typeof meal !== 'object') return false;
      if (!Array.isArray(meal.components) || meal.components.length === 0) {
        logStep("Skipping invalid meal - no components", { name: meal?.name });
        return false;
      }
      return true;
    });

    logStep("Valid meals after filtering", { count: validMeals.length });
